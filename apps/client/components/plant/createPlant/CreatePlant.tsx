...
export const CreatePlant: React.FC<Props> = ({ onClose, mode, plantId, fetchAgain }) => {
  ...
  const handleWateringInterval = (value: number) => {
    setPlant({
      ...plant,
  ...
  const validateInputFields = () => {
    if (
      plantNickname.current.value === '' ||
      plantNickname.current.value === undefined ||
      plantName.current.value === '' ||
      plantName.current.value === undefined ||
      plantLatinName.current.value === '' ||
      plantLatinName.current.value === undefined ||
      plantImage.current.value === '' ||
      plantImage.current.value === undefined
    ) {
      ...
      <div className={styles.plantWrapper}>
            ...
            <div className={styles.formRows}>
              <div>
                <InputField
                  ...
                  type={'number'}
                  onChange={(e) => handleIdealEnvironment(parseInt(e.target.value), 'mintemp')}
                  placeholder={'50'}
                />
              </div>
              <div>
                <InputField
                  ...
                  type={'number'}
                  onChange={(e) => handleIdealEnvironment(parseInt(e.target.value), 'maxtemp')}
                  placeholder={'200'}
                />
              </div>
            </div>
            <div className={styles.formRows}>
              <div>
                <InputField
                  label={'Min humidity (%)'}
                  type={'number'}
                  onChange={(e) => handleIdealEnvironment(parseInt(e.target.value), 'minhum')}
                  placeholder={'20'}
                />
              </div>
              <div>
                <InputField
                  label={'Max humidity (%)'}
                  type={'number'}
                  onChange={(e) => handleIdealEnvironment(parseInt(e.target.value), 'maxhum')}
                  placeholder={'40'}
                />
              </div>
            </div>
            <div className={styles.formRows}>
              <div>
                <InputField
                  label={'Min CO2 (g/m3)'}
                  type={'number'}
                  onChange={(e) => handleIdealEnvironment(parseInt(e.target.value), 'minco2')}
                  placeholder={'10'}
                />
              </div>
              <div>
                <InputField
                  label={'Max CO2 (g/m3)'}
                  type={'number'}
                  onChange={(e) => handleIdealEnvironment(parseInt(e.target.value), 'maxco2')}
                  placeholder={'20'}
                />
              </div>
            </div>
        ...
        <div className={styles.wateringIntervalContainer}>
          ...
          <Slider
            maxValue={14}
            minValue={0}
            onChange={handleWateringInterval}
            defaultValue={7}
            unit={' days'}
            ariaLabel={'days-slider'}
            step={1}
          />
        </div>
        <p className={styles.errorLabel}>{errorLabel}</p>
        <div className={styles.actionBtnContainer}>
          <SecondaryButtonSmall onClick={onClose}>
            <p>{mode === 'create' ? 'Cancel' : 'Discard changes'}</p>
          </SecondaryButtonSmall>
          <PrimaryButtonSmall onClick={() => onSubmit(mode)}>
            <p>{mode === 'create' ? 'Add plant' : 'Apply changes'}</p>
          </PrimaryButtonSmall>
        </div>
      </div>
    </div>
